...
function getWorkspacePackageDirs(rootDir: string): string[] {
    ...
    // This repo uses "packages/*". Keep it intentionally minimal and explicit.
    const dirs: string[] = [];
    for (const p of patterns) {
        if (p !== "packages/*") continue;
        const pkgsDir = joinPath(rootDir, "packages");
        for (const entry of readdirSync(pkgsDir, { withFileTypes: true })) {
            if (entry.isDirectory()) dirs.push(joinPath(pkgsDir, entry.name));
        }
    }
    return dirs;
}
